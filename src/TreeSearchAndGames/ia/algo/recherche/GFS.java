...
/**
 * Implémentation de l'algorithme de recherche gloutonne (Greedy Best-First Search)
 *
 * GFS explore les nœuds dans l'ordre croissant de l'heuristique h(n).
 * Utilise une file de priorité ordonnée par h(n) = estimation du coût au but.
 *
 * Proprietes :
 * - Complet : Non (peut boucler dans certains cas)
 * - Optimal : Non (ne considere pas le cout du chemin deja parcouru)
 * - Complexite temps : O(b^m) dans le pire cas
 * - Complexite espace : O(b^m)
 */
public class GFS extends TreeSearch {

    /**
     * Crée un algorithme GFS (Greedy Best-First Search)
    ...
    public GFS(SearchProblem prob, State initial_state) {
        ...
        // File de priorité ordonnée par l'heuristique h(n) seulement
        this.frontier = new PriorityQueue<SearchNode>(
            Comparator.comparingDouble(SearchNode::getHeuristic)
        );
    }

    /**
     * Résout le problème en utilisant la recherche gloutonne
    ...
    public boolean solve() {
        ...
        while (!this.frontier.isEmpty()) {
            ...
            // 5. Retirer le noeud avec la PLUS PETITE HEURISTIQUE de la frontière
            ...
            if (ArgParse.DEBUG)
                System.out.print(cur_state + " (h=" + cur_node.getHeuristic() + ", " + actions.size() + " actions) -> {");
            ...
            for (Action a : actions) {
                ...
                if (ArgParse.DEBUG)
                    System.out.print("(" + a + ", " + child_state + ", h=" + child_node.getHeuristic() + ")");

                // 11. S'il n'a pas été visité
                if (!explored.contains(child_state)) {
                    // Vérifier s'il est déjà dans la frontière
                    ...
                    if (existing == null) {
                        ...
                        if (ArgParse.DEBUG)
                            System.out.print("[A] ");
                    } else if (child_node.getHeuristic() < existing.getHeuristic()) {
